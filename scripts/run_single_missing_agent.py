...
(e.g. one that was skipped mid-panel by a transient rate-limit/session-cap
error), without re-calling the provider for every other already-completed
agent, then regenerate the survey's aggregate report from all agents'
on-disk accepted samples. Usage:

    PYTHONPATH=src python3 scripts/run_single_missing_agent.py <survey_dir> <agent_id>
"""
