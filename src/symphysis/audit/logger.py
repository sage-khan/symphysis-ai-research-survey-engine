...
Moved here from the top-level `storage.py` as part of the governance-layer
extraction (`docs/architecture/governance-layer-and-runtime-backends-plan.md`
Phase 0): this module is the audit trail's write path, one of the three
pillars (identity/policy/audit) that layer organizes existing code around.
The on-disk contract above is unchanged by the move.
"""
