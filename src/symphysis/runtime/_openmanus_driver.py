...
def _build_instrument_submit_flat_proxy_tool_class(ProxyTool):
    """2026-09-02: the flat single-comparison-set counterpart to
    InstrumentSubmitProxyTool above, for the default (non-panel)
    ToolCallAgent loop in _run() below. Same bookkeeping need for the same
    reason (the driving while-loop needs to know PROGRAMMATICALLY when a
    genuine, schema-valid instrument_submit call has landed, not just
    whatever the agent's last memory message happens to be, which could be
    a stray thought or a text answer that dodged the tool call entirely).
    No level_id here: a flat survey has exactly one comparison set, so
    `accepted`/`last_attempt` are single dicts, not level_id-keyed."""
    from pydantic import Field

    class FlatInstrumentSubmitProxyTool(ProxyTool):
        accepted: dict = Field(default_factory=dict)
        last_attempt: dict = Field(default_factory=dict)

        async def execute(self, **kwargs):
            result = await super().execute(**kwargs)
            self.last_attempt = dict(kwargs)
            if result.error is None and result.output:
                try:
                    parsed = json.loads(result.output)
                except (TypeError, ValueError):
                    parsed = None
                if parsed and parsed.get("valid"):
                    self.accepted = parsed.get("answer") or dict(kwargs)
            return result

    return FlatInstrumentSubmitProxyTool
...
async def _run(spec: dict) -> None:
    from app.agent.toolcall import TOOL_CALL_REQUIRED
    ...
    # 2026-09-02: tracks the flat (non-panel) instrument_submit tool
    # instance, if one is built below, so the while-loop after this block
    # can check its `.accepted` after every step and stop the moment a
    # genuine, schema-valid tool call lands -- instead of the old behavior
    # of running until Terminate/max_steps and grabbing whatever the LAST
    # memory message happened to be, tool call or not. See
    # agentic-experiment-design-decisions.md (project-veritas) 2026-09-02
    # entry for why this exists: a flat bwm survey previously either never
    # reached this loop at all (routed to bwm_two_stage) or, before that,
    # had no programmatic way to tell a real instrument_submit call apart
    # from a stray final text message.
    flat_submit_tool = None
    if proxy_port and available_tools:
        ...
        # (see _build_instrument_submit_proxy_tool_class's docstring); in
        # the flat (non-panel) case it gets the flat counterpart instead,
        # if the caller registered one (agent.py's tools/registry.py only
        # does this for runtime_backend="openmanus_react" -- see there).
        ...
        FlatInstrumentSubmitProxyTool = _build_instrument_submit_flat_proxy_tool_class(ProxyTool) if not is_panel else None
        proxy_tools = []
        for t in available_tools:
            if is_panel and t["name"] == "instrument_submit":
                tool_cls = InstrumentSubmitProxyTool
            elif not is_panel and t["name"] == "instrument_submit":
                tool_cls = FlatInstrumentSubmitProxyTool
            else:
                tool_cls = ProxyTool
            instance = tool_cls(
                name=t["name"],
            ...
            if tool_cls is FlatInstrumentSubmitProxyTool:
                flat_submit_tool = instance
            proxy_tools.append(instance)
    ...
    # 2026-09-02: RAG-enabled agents (two retrieval tool calls --
    # rag_retrieval for the dedicated corpus, then knowledge_repo_retrieval
    # for shared knowledge -- before the model is expected to call
    # instrument_submit) were observed to fail 100% of the time
    # (6/6 *-openmanus RAG agents, 0/18 accepted) on a real L2 run, with
    # every failure raising ToolCallAgent.act()'s hard
    # `ValueError(TOOL_CALL_REQUIRED)` (vendor/openmanus/app/agent/
    # toolcall.py): by the third step, after two tool observations have
    # been appended to its own context, qwen3:14b's Ollama-emulated
    # function-calling stopped emitting the tool-call format entirely and
    # replied with plain text instead, which ToolChoice.REQUIRED treats as
    # a hard error rather than a recoverable one -- ending the whole
    # attempt outright with no accepted sample and no retry. Matched
    # base agents (one retrieval, one submit) never hit this: the failure
    # tracks step/context-length, not RAG content specifically. This is
    # the direct-tool-calling-path analogue of the older, already-fixed
    # "RAG context made qwen3:14b answer with full labels instead of bare
    # codes" bug in instruments/bwm.py's plain-completion flow (see
    # docs/development/diagnostics.md) -- same underlying cause (a later,
    # longer turn loses track of an earlier-stated hard requirement), a
    # different failure mode because this path enforces the requirement
    # via the tool-calling API rather than parsing free text. The fix is
    # the same principle applied at this failure's own point of
    # generation: catch the hard error here (vendor code raises it in
    # act(), by which point think() has already appended the model's
    # failed plain-text attempt to memory) and inject one corrective
    # reminder message before retrying, instead of letting one dropped
    # tool call end the whole sample. Bounded at 3 consecutive recoveries
    # so a genuinely broken model/task still terminates via max_steps
    # rather than looping forever.
    consecutive_tool_call_recoveries = 0
    while agent.state != AgentState.FINISHED and agent.current_step < agent.max_steps:
        pre_len = len(agent.memory.messages)
        try:
            result = await agent.step()
        except ValueError as exc:
            if str(exc) != TOOL_CALL_REQUIRED or consecutive_tool_call_recoveries >= 3:
                raise
            consecutive_tool_call_recoveries += 1
            agent.current_step += 1
            agent.memory.add_message(
                Message.user_message(
                    "Your previous reply had no tool call in it. You MUST call "
                    "instrument_submit (or Terminate) with your answer as the "
                    "tool call's arguments -- do not restate the answer as "
                    "plain text. Call the tool now."
                )
            )
            _emit(
                "tool_call_required_recovery",
                {"step": agent.current_step, "consecutive_recoveries": consecutive_tool_call_recoveries},
            )
            continue
        agent.current_step += 1
        consecutive_tool_call_recoveries = 0
        ...
        # A genuine, schema-valid instrument_submit call is itself the
        # answer: stop here rather than continuing to step (which would, at
        # best, waste a step on a now-redundant Terminate call, and at
        # worst let the model second-guess a valid answer into an invalid
        # one). This is the actual fix that makes the emitted raw_completion
        # traceable to a real tool call instead of "whatever the last memory
        # message happened to be".
        if flat_submit_tool is not None and flat_submit_tool.accepted:
            break

    if flat_submit_tool is not None and flat_submit_tool.accepted:
        final_message = json.dumps(flat_submit_tool.accepted)
    elif flat_submit_tool is not None and flat_submit_tool.last_attempt:
        # No accepted submission, but a real (invalid) instrument_submit
        # attempt exists: surface THAT, not a random text message, so
        # guardrails.py's existing reject-and-repair retry loop sees the
        # model's actual attempted answer and its actual validation errors,
        # same as any other rejected-but-real sample.
        final_message = json.dumps(flat_submit_tool.last_attempt)
    else:
