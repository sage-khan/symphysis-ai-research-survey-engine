...
## Manual provider's index inference skipped a just-answered sample

**Found:** writing `tests/test_manual_provider.py`.
...
**Fix:** replaced file-state inference with an explicit per-directory call
counter (`providers/manual_provider.py`), so N sequential calls in one
